-- ============================================
-- Seed Data untuk User - Comprehensive Demo Data
-- Migration: 006_seed_data.sql
-- User ID: GANTI_MANUAL_USER_ID_DISINI
-- ============================================
-- 
-- CATATAN: Ganti v_user_id dengan user_id yang sesuai sebelum menjalankan script ini
-- Script ini membuat data lengkap untuk demonstrasi semua fitur aplikasi:
-- - Transactions untuk 3 bulan terakhir dengan berbagai pattern
-- - Budgets dengan berbagai status (aman, hampir habis, melebihi)
-- - Goals dengan berbagai progress (baru mulai, on track, hampir selesai, completed)
-- - Goal history yang lengkap dan realistis
-- ============================================

-- Set user ID variable (GANTI INI DENGAN USER_ID YANG BENAR)
DO $$
DECLARE
...
-- ============================================
-- 1. INSERT TRANSACTIONS (3 Bulan Terakhir)
-- ============================================
-- Pattern: Bulan ini aktif, bulan lalu banyak transaksi, 2 bulan lalu normal

-- BULAN INI (Current Month) - Data Aktif & Bervariasi
...
-- BULAN LALU (Previous Month) - Banyak Transaksi (Budget Hampir Habis)
INSERT INTO public.transactions (user_id, date, category, amount, note) VALUES
  -- Transaksi bulan lalu (menggunakan DATE_TRUNC untuk memastikan di bulan lalu)
...
-- 2 BULAN LALU (Previous Month 2) - Data Normal
INSERT INTO public.transactions (user_id, date, category, amount, note) VALUES
...
-- ============================================
-- 2. INSERT BUDGETS (3 Bulan dengan Berbagai Status)
-- ============================================

-- BULAN INI - Beberapa kategori hampir habis, beberapa masih aman
...
-- BULAN LALU - Beberapa kategori melebihi budget (untuk menunjukkan warning)
INSERT INTO public.budgets (user_id, category_id, category_name, year_month, budget_amount, spent_amount, color) VALUES
...
-- Update spent_amount dari transactions untuk semua bulan
...
SELECT 
  ...
  'Top up pertama'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'Beli Laptop Gaming'
LIMIT 1;
...
-- History untuk "Liburan ke Bali" (on track)
INSERT INTO public.goal_history (goal_id, user_id, amount, date, note)
SELECT 
  g.id,
  v_user_id,
  ...
  'Tabungan awal'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'Liburan ke Bali'
LIMIT 1;
...
-- History untuk "Emergency Fund" (baru mulai)
INSERT INTO public.goal_history (goal_id, user_id, amount, date, note)
SELECT 
  g.id,
  v_user_id,
  ...
  'Tabungan awal'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'Emergency Fund'
LIMIT 1;
...
-- History untuk "Beli Sepeda" (completed)
INSERT INTO public.goal_history (goal_id, user_id, amount, date, note)
SELECT 
  g.id,
  v_user_id,
  ...
  'Tabungan awal'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'Beli Sepeda'
LIMIT 1;
...
SELECT 
  ...
  1500000,
  CURRENT_DATE - INTERVAL '5 days',
  'Top up final - Goal tercapai!'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'Beli Sepeda'
LIMIT 1;

-- History untuk "Bayar Tagihan Kampus" (urgent, progress kurang)
INSERT INTO public.goal_history (goal_id, user_id, amount, date, note)
SELECT 
  g.id,
  v_user_id,
  ...
  'Tabungan awal'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'Bayar Tagihan Kampus'
LIMIT 1;
...
-- History untuk "DP Rumah" (jangka panjang)
INSERT INTO public.goal_history (goal_id, user_id, amount, date, note)
SELECT 
  g.id,
  v_user_id,
  ...
  'Tabungan awal'
FROM public.goals g
WHERE g.user_id = v_user_id AND g.title = 'DP Rumah'
LIMIT 1;
...
-- Update saved amount dari goal_history untuk semua goals
...
-- VERIFIKASI DATA
-- ============================================
-- Uncomment untuk melihat summary data yang di-insert:
-- SELECT 'Transactions' as table_name, COUNT(*) as count FROM public.transactions WHERE user_id = 'GANTI_MANUAL_USER_ID_DISINI'
-- UNION ALL
-- SELECT 'Budgets', COUNT(*) FROM public.budgets WHERE user_id = 'GANTI_MANUAL_USER_ID_DISINI'
-- UNION ALL
-- SELECT 'Goals', COUNT(*) FROM public.goals WHERE user_id = 'GANTI_MANUAL_USER_ID_DISINI'
-- UNION ALL
-- SELECT 'Goal History', COUNT(*) FROM public.goal_history WHERE user_id = 'GANTI_MANUAL_USER_ID_DISINI';
